...
function DarkConfigMenu:draw()
    ...
    if self.state ~= "CONTROLS" then
        ...
        Draw.draw(self.heart_sprite, 71, 56 + ((self.currently_selected - 1) * 32), Game:getSoulRotation(), 1, 1, 8, 8)
    else
        ...
        if dualshock then
            Draw.draw(self.heart_sprite, 6, 42 + ((self.currently_selected - 1) * 29), Game:getSoulRotation(), 1, 1, 8, 8)
        else
            Draw.draw(self.heart_sprite, 6, 42 + ((self.currently_selected - 1) * 28) + 2, Game:getSoulRotation(), 1, 1, 8, 8)
        end
    end
