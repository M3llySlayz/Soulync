...
local better_battles_installed = false

function BattleUI:drawState()
	if Mod.libs["better_battles"] then
		better_battles_installed = true
	end
	local rot = Game.battle.encounter:getSoulRotation()
    if Game.battle.state == "MENUSELECT" then
		...
		if better_battles_installed then
			...
			local size = {(heart_sprite:getWidth()/2), (heart_sprite:getHeight()/2)}
			Draw.draw(heart_sprite, 5 + ((Game.battle.current_menu_x - 1) * 230) + size[1], 30 + ((Game.battle.current_menu_y - (page*3)) * 30) + size[2], rot, 1, 1, size[1], size[2])
		else
			Draw.setColor(Game.battle.encounter:getSoulColor())
			Draw.draw(self.heart_sprite, 13 + ((Game.battle.current_menu_x - 1) * 230), 38 + ((Game.battle.current_menu_y - (page*3)) * 30), rot, 1, 1, 8, 8)
		end
    ...
    elseif Game.battle.state == "ENEMYSELECT" or Game.battle.state == "XACTENEMYSELECT" then
		...
		if better_battles_installed then
			...
			local size = {(heart_sprite:getWidth()/2), (heart_sprite:getHeight()/2)}
			Draw.draw(heart_sprite, 55 + size[1], 30 + ((Game.battle.current_menu_y - page_offset) * 30) + size[2], rot, 1, 1, size[1], size[2])
		else
        Draw.setColor(Game.battle.encounter:getSoulColor())
        Draw.draw(self.heart_sprite, 63, 38 + ((Game.battle.current_menu_y - page_offset) * 30), rot, 1, 1, 8, 8)
		end
        ...
        if not better_battles_installed then
